#!/usr/bin/env bash
set -euo pipefail
printf '%s\n' '🔐 Applying Packet Atlas v2.3 — SSH Connection Journey...'
if [ ! -f package.json ] || [ ! -d src/features/packet-atlas ]; then echo '❌ Run this script from the inner Packet Atlas app folder.'; exit 1; fi
if [ ! -f src/features/packet-atlas/scenarios/scenarioRegistry.ts ]; then echo '❌ Apply v2.2 Scenario Authoring System first.'; exit 1; fi
mkdir -p patches/backups/v2.3 tests/unit
cp src/features/packet-atlas/scenarios/scenarioRegistry.ts patches/backups/v2.3/scenarioRegistry.ts 2>/dev/null || true
cp src/features/packet-atlas/PacketAtlasPage.tsx patches/backups/v2.3/PacketAtlasPage.tsx 2>/dev/null || true

cat > src/features/packet-atlas/scenarios/sshConnectionScenario.ts <<'TS'
...
text=page.read_text()
text=re.sub(r'Packet Atlas v[0-9.]+','Packet Atlas v2.3',text)
page.write_text(text)
PY2

cat > tests/unit/sshScenario.test.ts <<'TS'
...
printf '%s\n' '✅ v2.3 applied — SSH Connection Journey.'
